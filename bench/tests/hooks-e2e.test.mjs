...
// Mutation intent is recorded by the CLI itself. The optional call-counter is
// still tested as a hook, but it is no longer part of the lean default install.
...
function installedHooks(options = {}) {
  ...
  if (options.callCounter) {
    execFileSync(process.execPath, [IPA_BIN, "harness", "update", "claude", "--with", "hook:call-counter"], {
      env: { ...process.env, IPA_VAULT_PATH: sandbox, IPA_HARNESS_HOME: home },
      encoding: "utf8"
    });
  }
...
test("CLI records a dry-run move and mutation_pending reads it; --apply clears it", () => {
  const { sandbox } = installedHooks();
  ...
  const runCli = (args) => execFileSync(process.execPath, [IPA_BIN, "--vault", sandbox, ...args], {
    env: { ...process.env, IPA_SESSION_ID: "s1" },
    encoding: "utf8"
  });
  runCli(["move", "Alpha", "01 Project"]);
  ...
  runCli(["move", "Alpha", "01 Project", "--apply"]);
...
test("call-counter hook counts each ipa call once and hook_call_count reads the un-doubled total", () => {
  const { sandbox, hooksDir } = installedHooks({ callCounter: true });
