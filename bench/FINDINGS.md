...
## F10 — 벤치가 개발자 개인 CLAUDE.md에 오염돼 있었다 → 순수 하네스 원칙 채택 [벤치 유효성 + 하네스 공백 1건]
훅 격리(F9 후속, CLAUDE_CONFIG_DIR) 과정에서 드러남: --settings 병합 시절 벤치 세션은 실-홈 훅뿐
아니라 **개발자 개인 ~/.claude/CLAUDE.md(작업 원칙)도 로드**하고 있었다. 즉 그동안의 pass 일부는
ipa 하네스가 아니라 개인 원칙 덕이었다.
- **증거(클린 표면 진단):** c12-inbox-triage가 opus·sonnet **양 모델 동시** correctness=false로 전환
  (승인 전 turn 1에서 인박스 이동 적용). 양 모델 동시 = flaky가 아니라 체계 신호. "되돌리기 어려운
  작업은 확인 먼저"가 개인 CLAUDE.md에서 오던 행동이었다.
- **원칙 결정: 순수 하네스 표면이 기준선.** 벤치는 ipa가 배포하는 것(하네스 프롬프트 블록·스킬·훅)만
  가진 에이전트를 측정한다. 중립 baseline CLAUDE.md는 기각 — 귀속을 다시 흐리고, 시나리오가 빨간불일
  때 하네스 대신 그 문서를 손보는 제2 하네스/gaming 통로가 되며, "문서 없음"이 신규 사용자의 실존
  프로파일이라 최악 케이스 보장이 배포물의 기준이어야 한다.
- **공백 2분기 규율:** 클린 표면에서 드러난 공백마다 — (a) IPA 방법론/메커니즘의 소유물이면 하네스가
  가르친다, (b) ipa와 무관한 일반 예절/개인 선호면 하네스에 넣지 않고 그 어서션을 시나리오에서 뺀다.
- **적용 사례:** c12 승인-게이트는 (a)로 판정 — ipa의 propose→apply 관용구(triage dry-run→--apply)의
  자연 확장이라 하네스 소유가 맞다. triage 스킬+Safe Writes에 blast-radius 스코핑으로 교육(단일 노트
  변형은 바로 apply[P9 유지], 다중/bulk 변형은 계획 표면화→확인→apply). 교육 후 클린 run에서 c12
  correct=true 복귀(sonnet PASS 20콜, opus correct=true). 반대로 b8의 한국어 응답 regex는 (b)로 판정 —
  응답 언어는 개인 선호라 어서션을 언어-불문으로 수정. g30[opus]의 used_command도 과잉 명세로 판정
  (note replace = 합법 CLI 메커니즘, 진짜 게이트는 no_hand_edit+end-state).
