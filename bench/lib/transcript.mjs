...
// ipa 호출 감지: Bash tool_use 수가 아니라 실제 `ipa <subcommand>` 명령 수를 센다.
// 셸 구분자(&&, ||, ;)로 나눈 뒤 각 세그먼트의 줄머리를 훑으므로, 한 Bash 호출에
// `ipa view A && ipa view B`가 있으면 2콜로 기록한다. `(^|\n)`로 heredoc 종료 뒤 새 줄에서
// 이어지는 `ipa ...`(예: `cat > f <<'EOF' ... EOF\nipa inbox add`)도 잡는다.
// 트레이드오프: heredoc 본문에 `ipa search ...`처럼 실제 명령 형태의 줄이 있으면 오탐할 수
// 있으나, 실제 `&& ipa <sub>` 체이닝을 놓치지 않는 쪽을 우선한다.
const IPA_COMMAND_RE = /(?:^|\n)\s*(ipa\s+[a-z-][^\n]*)/g;

export function extractIpaCommands(command) {
  const found = [];
  for (const segment of String(command ?? "").split(/&&|\|\||;/)) {
    for (const match of segment.matchAll(IPA_COMMAND_RE)) found.push(match[1].trim());
  }
  return found;
}
...
export function parseTranscript(jsonlText) {
  ...
  out.ipaCalls = out.bashCalls.flatMap((call) => {
    const commands = extractIpaCommands(call.command);
    return commands.map((command, index) => ({
      id: commands.length === 1 ? call.id : `${call.id}:${index + 1}`,
      bashId: call.id,
      command,
