"""G7 engine_loss tests — SearchEngine-driven evaluation."""
...
from ipa_cli.tune.engine_loss import (
    compute_loss_v2,
...
def test_regression_hit_at_rank_1() -> None:
    ...
    hit, rank = evaluate_regression_v2(
        case, engine, weights=None, threshold=0.0, cap=10
    )
    assert hit is True
    assert rank == 1
...
def test_regression_miss_when_target_below_threshold() -> None:
    ...
    hit, _ = evaluate_regression_v2(case, engine, None, threshold=0.10, cap=10)
    assert hit is False
...
def test_regression_miss_when_target_below_cap() -> None:
    ...
    hit, _ = evaluate_regression_v2(case, engine, None, threshold=0.0, cap=2)
    assert hit is False
...
def test_multi_query_sums_per_note_scores() -> None:
    ...
    hit, rank = evaluate_regression_v2(case, engine, None, threshold=0.0, cap=10)
    assert hit is True
    assert rank == 1
...
def test_scenario_recall_threshold_requires_min_targets() -> None:
    ...
    hit, rank = evaluate_scenario_v2(case, engine, None, threshold=0.0, cap=10)
    assert hit is True
    assert rank == 1  # best matched rank

    # Now require 3 of {A,B,Z}: only A and B available → miss.
    case_strict = {**case, "recall_threshold": 3}
    hit2, _ = evaluate_scenario_v2(case_strict, engine, None, 0.0, 10)
...
def test_scenario_top_n_limits_visibility() -> None:
    ...
    hit, _ = evaluate_scenario_v2(case, engine, None, 0.0, 10)
    assert hit is False


def test_compute_loss_v2_aggregates_misses_with_penalties() -> None:
    ...
    loss, m = compute_loss_v2(engine, regression, scenario, None, 0.0, 10)
...
def test_weights_change_alters_ordering() -> None:
    ...
    # Boost ch1 → A wins.
    hit_a, rank_a = evaluate_regression_v2(
    ...
    hit_b, rank_b = evaluate_regression_v2(
