...
```js
{
  ...
  harness: true,                   // 세션 전 샌드박스에 harness install 할지 (F18/F20은 false)
  models: ["sonnet", "opus"],
...
**Interfaces:**
- Produces:
  - `createSandbox(personaDir, label) → string` — tmpdir에 재귀 복사 후 `.ipa-config`(`vault_path: <dir>`) 작성, 경로 반환
...
export function createSandbox(personaDir, label) {
  const dir = mkdtempSync(join(tmpdir(), `ipa-bench-${label}-`));
  cpSync(personaDir, dir, { recursive: true });
  writeFileSync(join(dir, ".ipa-config"), `vault_path: ${dir}\n`);
  return dir;
}
...
- Produces:
  - `runClaudeTurn({ cwd, model, message, resumeSessionId?, maxTurns?, claudeCmd?, timeoutMs? }) → Promise<string>` — raw stream-json 텍스트. `claudeCmd`는 `["claude"]` 기본, 테스트에서는 `[process.execPath, <fake-claude 경로>]`. 출력에 `"type":"result"` 라인이 있으면 exit code와 무관하게 resolve, 없으면 reject.
...
export function runClaudeTurn({ cwd, model, message, resumeSessionId = null, maxTurns = 12, claudeCmd = ["claude"], timeoutMs = 900_000 }) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(claudeCmd[0], args, { cwd, env: { ...process.env, IPA_VAULT_PATH: cwd } });
    let out = "", err = "";
...
async function runOne({ scenario, model }, args, runDir) {
  ...
  const sandbox = createSandbox(personaDir, scenario.id);
...
- Produces: `loadScenarios()`가 20개 시나리오 반환 (smoke 6, holdout 2, multi 8)
...
test("catalog: 20 scenarios, 8 multi, 6 smoke, 2 holdout, groups A-F covered", async () => {
  const all = await loadScenarios(DIR);
  assert.equal(all.length, 20);
  assert.equal(all.filter((s) => s.mode === "multi").length, 8);
...
Expected: PASS. 카운트가 안 맞으면 smoke/holdout/multi 플래그를 재점검한다 (smoke 6 = a1, b5, c9, c12, d13, e16 / holdout 2 = a4, f19 / multi 8 = c10, c12, d14, e16, e17, f18, f19, f20).

Run: `node bench/run.mjs --dry-run --full`
Expected: 18개(홀드아웃 제외) 시나리오가 fake-claude로 전 파이프라인을 통과하고 `run-summary.json`이 생성된다. 어서션 실패는 정상(fake transcript는 대부분의 expect를 만족하지 못함) — 확인 대상은 크래시·infra error 0건.
...
git commit -m "feat(bench): 시나리오 카탈로그 20종 — 인식·읽기·쓰기·강건성·저작·마이그레이션"
```
...
pnpm bench --full                  # 18종 × 시나리오별 모델 — 변경 채택 직전
...
- 스펙 커버리지: 3계층 분리(T1), 러너/스트림 파싱(T2·4), 샌드박스 격리(T3), 어서션 15종(T5), smoke/full/baseline/exit-gate(T6), 페르소나 5종(T7 — 스펙의 4종 + 빈 볼트), 카탈로그 20종·패러프레이즈·홀드아웃(T8), CLAUDE.md 게이트 규칙(T9). 스펙의 `scenarios/*.yaml`은 의존성 0 원칙에 따라 `.mjs`로 조정(Global Constraints에 명시).
