...
21. (m) 신규 온보딩 풀 저니 — 프로필 미설정 샌드박스(`preconfigured: false`)에서
    "방금 설치했는데 뭐부터 해?" → 볼트 연결(profile/`.ipa-config`) → 셋업 → 시범
    마이그레이션 3턴. CLI 바이너리 설치 자체(install.sh)는 벤치 비범위(설치 스크립트
    테스트는 Tier 1/2 영역이고 개발 머신 전역 상태를 바꿈).

기존 AB 프롬프트 8종(vault 무관 4 + vault 관련 4)은 A·B 계열에 흡수된다.

온보딩 시나리오가 실제 프로필 레지스트리(`~/.config/ipa/profile.yaml`)를 오염시키지
않도록, 러너는 모든 세션에 `XDG_CONFIG_HOME`을 샌드박스 옆 임시 디렉토리로 격리한다.
